...
module t07_matrix_mult (
    ...
    t07_wallace_tree wt_1(
        .a(parallel_out1[3:0]),
        .b(parallel_out1[7:4]),
        .product(product1)
    );

    t07_wallace_tree wt_2(
        .a(parallel_out2[3:0]),
        .b(parallel_out2[7:4]),
        .product(product2)
    );
...
module t07_rcv_block (
    ...
    t07_rcu control(.clk(clk), .n_rst(n_rst),
    ...
    t07_rx_data_buff data_buff(.clk(clk), .n_rst(n_rst),
    ...
    t07_start_bit_det sbd(.clk(clk), .n_rst(n_rst), 
                      .serial_in(serial_in), .new_packet_detected(new_packet_detected));
    
    t07_stop_bit_chk sbc(.clk(clk), .n_rst(n_rst),
                     .sbc_clear(sbc_clear), .sbc_enable(sbc_enable),
...
module t07_timer (
    ...
    logic [3:0] bit_cnt;
    logic [13:0] clk_cnt;
    t07_flex_counter #(.SIZE(14)) clock_count (.clk(clk),
                              .n_rst(n_rst),
                              .clear(pckt_done),
    ...
    t07_flex_counter #(.SIZE(4)) bit_count (.clk(clk),
                            .n_rst(n_rst),
...
module t07_flex_counter #(parameter SIZE = 4) (
    input logic clk,
    input logic n_rst,
    input logic clear,
...
module t07_sr_9bit (
    ...
    t07_flex_sr #(.SIZE(9), .MSB_FIRST(0)) sr_9 (
                  .clk(clk),
                  .n_rst(n_rst),
...
module t07_tx_block (
   ...
   output logic         tx_busy    
    ...
    /* verilator lint_off UNUSEDSIGNAL */
    logic [13:0] clk_cnt;
    t07_flex_counter #(.SIZE(14)) count (.clk(clk),
                              .n_rst(n_rst),
                              .clear(count_start),
                              .count_enable(1'b1),
                              .rollover_val(bit_period),
    ...
    t07_tcu tx_ctrl (.clk(clk), .n_rst(n_rst), .tx_data(tx_data), 
...
module t07_wallace_tree (
    ...
    logic s1, c1;
    t07_full_adder fa1(.a(pp0[1]), .b(pp1[0]), .carry_in(1'b0), .sum(s1), .carry_out(c1));
    assign product[1] = s1;

    logic s2, c2, s2_final, c2_ha;
    t07_full_adder fa2(.a(pp0[2]), .b(pp1[1]), .carry_in(pp2[0]), .sum(s2), .carry_out(c2));
    t07_half_adder ha2(.a(s2), .b(c1), .sum(s2_final), .carry_out(c2_ha));
    assign product[2] = s2_final;
    
    logic col2_carry;
    t07_full_adder fa2_carry(.a(c2), .b(c2_ha), .carry_in(1'b0), .sum(col2_carry), .carry_out());

    logic s3_1, c3_1, s3, c3;
    t07_full_adder fa3_1(.a(pp0[3]), .b(pp1[2]), .carry_in(pp2[1]), .sum(s3_1), .carry_out(c3_1));
    t07_full_adder fa3_2(.a(s3_1), .b(pp3[0]), .carry_in(col2_carry), .sum(s3), .carry_out(c3));
    assign product[3] = s3;
    
    logic col3_carry;
    t07_full_adder fa3_carry(.a(c3_1), .b(c3), .carry_in(1'b0), .sum(col3_carry), .carry_out());

    logic s4, c4, s4_final, c4_ha;
    t07_full_adder fa4(.a(pp1[3]), .b(pp2[2]), .carry_in(pp3[1]), .sum(s4), .carry_out(c4));
    t07_half_adder ha4(.a(s4), .b(col3_carry), .sum(s4_final), .carry_out(c4_ha));
    assign product[4] = s4_final;

    logic col4_carry;
    t07_full_adder fa4_carry(.a(c4), .b(c4_ha), .carry_in(1'b0), .sum(col4_carry), .carry_out());

    logic s5, c5;
    t07_full_adder fa5(.a(pp2[3]), .b(pp3[2]), .carry_in(col4_carry), .sum(s5), .carry_out(c5));
    assign product[5] = s5;

    logic s6, c6;
    t07_half_adder ha6(.a(pp3[3]), .b(c5), .sum(s6), .carry_out(c6));
    assign product[6] = s6;
